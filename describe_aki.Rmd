...
outc_bin <- outc[, .(label = max(label), onset = if(any(label == 1)) min(time[label == 1]) + 6 else NA_real_), by = c("src", "stay_id")]
...
expect_equal(max(count(dyn, src, stay_id)$n), 175)       # no more than 193 time steps (1 + 7 * 24 + 24)
...
gbar %+% outc_bin %+% aes(x=onset) + labs(title="Onset of sepsis")
gbar %+% outc_bin[onset <= 36] %+% aes(x=onset) + labs(title="Onset of sepsis in first 24h")
