...
all_missing <- dyn[, map(.SD, ~ all(is.na(.))), by = c("src", "stay_id"), .SDcols = c(vitals)]
tbl_summary(all_missing %>% select(-stay_id), by="src")

# How is this missingness jointly distributed? Are there blocks of missingness in some patients?
all_missing <- all_missing[, c(.(src = src), map(.SD, ~ ifelse(., NA, FALSE))), .SDcols = c(vitals)]
...
Vital signs are also measured approximately every hour in almost all databases. Temperature is again the exception across databases, most likely because it is entered by nurses. However, while it is measured every 2-4 hours in AUMC, eICU, and MIMIC, it is only measured every 6 hours in HiRID.
...
ICU mortality is comparable between eICU, HiRID, and MIMIC, hovering between 5.5-8.2%. AUMCdb on the other hand has more than twice the ICU mortality rate at 16%. 
