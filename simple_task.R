...
path <- file.path(conf$output_dir, "simple")
...
# Apply exclusions
patients <- exclude(patients, mget(paste0("excl", 6)))
...
# Get predictors
dyn <- load_step(dict[dynamic_vars], cache = TRUE, interval = time_unit(freq))
sta <- load_step(dict[static_vars], cache = TRUE, interval = time_unit(freq))
